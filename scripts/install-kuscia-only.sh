...
function init_deploy_shell() {
	...
	docker run --rm "$KUSCIA_IMAGE" cat "${CTR_ROOT}"/scripts/deploy/kuscia.sh >"$I_PATH"/kuscia.sh && chmod u+x "$I_PATH"/kuscia.sh
	docker run --rm "$KUSCIA_IMAGE" cat "${CTR_ROOT}"/scripts/deploy/register_app_image.sh >"$I_PATH"/register_app_image_0.sh && chmod u+x "$I_PATH"/register_app_image_0.sh
}
