...
- [8. 常见问题排查](#8-常见问题排查)
- [9. 对接本地源码编译的 Kuscia（非 Docker）](#9-对接本地源码编译的-kuscia非-docker)
- [10. 附录：端口与文件速查](#10-附录端口与文件速查)
...
## 9. 对接本地源码编译的 Kuscia（非 Docker）

> 适用场景：希望 SecretPad 后端直接调用本地源码编译的 Kuscia 二进制，而不是通过 Docker 启动 Kuscia 容器。
...
SecretPad 后端（源码启动）
        │ gRPC / notls
        ▼
本地 Kuscia 二进制（master + lite alice/bob）
        │
        └── 无需 Docker 容器运行 Kuscia
```

### 9.2 Kuscia 侧修改（t1 分支）

Kuscia 默认把根目录硬编码为 `/home/kuscia`，普通用户无法写入。为了让 Kuscia 可以在任意目录运行，已在 `kuscia` 仓库 `t1` 分支做如下修改：

- 文件：`pkg/common/constants.go`
- 修改：`DefaultKusciaHomePath` 优先读取环境变量 `KUSCIA_HOME`，未设置时回退到 `/home/kuscia`。
...
### 9.3 本地运行 Kuscia master（需要 root）

> ⚠️ Kuscia 内部的 CoreDNS 会监听 53 端口，必须以 root 用户运行。
...
# 将配置模板中的 /home/kuscia 替换为实际 KUSCIA_HOME
...
为方便使用，kuscia 仓库 `t1` 分支已提供一键脚本：

```bash
cd /home/charles/code/sfwork/kuscia
sudo bash scripts/run-local-master.sh [KUSCIA_HOME_DIR]
```

### 9.4 SecretPad 侧修改（t1 分支）

SecretPad 后端默认通过容器名（`root-kuscia-master` 等）连接 Kuscia。在 `secretpad` 仓库 `t1` 分支中，`config/application-dev.yaml` 已改为默认连接本机端口：
...
本地 Kuscia 运行后，在 `secretpad` 仓库执行：
...
1. `bash scripts/test/setup.sh` — 生成证书、数据库
2. `mvn clean install -Dmaven.test.skip=true` — 编译后端
3. 以 dev profile 启动后端，连接 `127.0.0.1:18083` 等本地 Kuscia 端口
...
# Kuscia master 端口检查
ss -tlnp | grep -E '18083|13081'
```

### 9.7 注意事项

1. **权限**：Kuscia 本地二进制运行需要 root，因为 CoreDNS 监听 53 端口、Envoy 监听 80 端口。
2. **Lite 节点**：master 启动后，仍需按 Kuscia 官方文档注册并启动 alice / bob lite 节点（步骤与 Docker 部署类似，只是使用本地二进制）。
3. **证书**：使用 `notls` 模式时，不需要配置 KusciaAPI 客户端证书；使用 `tls`/`mtls` 时，需确保证书与本地 Kuscia 一致。
4. **端口冲突**：如果本地 Kuscia 使用非默认端口，请同步修改 `config/application-dev.yaml` 或通过环境变量覆盖。
