...
- [1. 环境准备](#1-环境准备)
- [2. 一键部署 Kuscia（Docker）](#2-一键部署-kusciadocker)
- [3. 后端服务启动](#3-后端服务启动)
- [4. 前端服务启动](#4-前端服务启动)
- [5. 登录与功能操作](#5-登录与功能操作)
- [6. Nginx / Tomcat / Kuscia 端口配置逻辑](#6-nginx--tomcat--kuscia-端口配置逻辑)
- [7. 常见问题排查](#7-常见问题排查)
- [8. 附录：端口与文件速查](#8-附录端口与文件速查)
...
### 2.1 仅部署 Kuscia（不部署容器版 SecretPad）
...
- 8080 被占用：使用其他端口作为 HTTP 端口，例如 18080：
...
### 7.2 后端启动报 `no main manifest attribute`
...
### 7.3 后端日志出现 `UnknownHostException: root-kuscia-master`
